...
export const TextInput: React.FC<TextInputProps> = ({ label, className, ...props }) => (
  <div className="flex flex-col space-y-1 mb-4">
    ...
    <input
      className={`border border-slate-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-imesc-400 focus:border-imesc-500 outline-none transition-all ${className}`}
      {...props}
    />
  </div>
);
...
export const TextArea: React.FC<TextAreaProps> = ({ label, quickInserts, onQuickInsert, className, ...props }) => {
  return (
    <div className="flex flex-col space-y-1 mb-6">
      ...
      <textarea
        className={`border border-slate-300 rounded-md px-3 py-2 text-sm min-h-[120px] focus:ring-2 focus:ring-imesc-400 focus:border-imesc-500 outline-none transition-all resize-y font-sans leading-relaxed ${className}`}
        {...props}
      />
    </div>
  );
